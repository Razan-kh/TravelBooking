...
public class GetHotelDetailsHandler : IRequestHandler<GetHotelDetailsQuery, Result<HotelDetailsDto>>
{
    ...
    public GetHotelDetailsHandler(IHotelRepository hotelRepo, IRoomRepository roomRepo, IReviewRepository reviewRepo, IMapper mapper)
    {
    ...
    public async Task<Result<HotelDetailsDto>> Handle(GetHotelDetailsQuery request, CancellationToken cancellationToken)
    {
        ...
        hotelDto.Reviews = _mapper.Map<IEnumerable<ReviewDto>>(reviews);
        ...
        {
            var dto = _mapper.Map<RoomCategoryDto>(cat);
